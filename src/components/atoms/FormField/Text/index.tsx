...
export class FormField extends React.Component<FormFieldProps<string | undefined>, IFormFieldState> {
  readonly state = {
    focused: false
  };
      ...
      {theme => {
        ...
        return (
          <span className={css({display: 'inline-flex'})}>
            ...
            <input type={this.props.type} placeholder={'Placeholder'} style={this.props.style}
                   className={cx(baseCSS, inputStyles, this.props.className)}
                   value={this.props.value ? (this.props.valueTransformer ? this.props.valueTransformer.toString(this.props.value) : this.props.value) : ''}
                   onChange={e => {
                     const val = e.target.value === '' ? null : e.target.value;

                     if (this.props.onChange) {
                       return this.props.onChange(val !== null ? (this.props.valueTransformer ? this.props.valueTransformer.fromString(val) : val as any) : null)
                     }
                   }}
                   onFocus={() => {
                     this.setState({
...
export function specifyFormType<V extends (string | string[] | number | undefined)>(type: string, valueTransformer?: ValueTransformer<V>): React.ComponentType<Omit<Omit<FormFieldProps<V>, "type">, "valueTransformer">> {

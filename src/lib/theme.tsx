...
type ThemeTypographyStyle = Required<Pick<React.CSSProperties, 'fontFamily' | 'fontSize'>> &
  Partial<
    Pick<
      React.CSSProperties,
      | 'letterSpacing'
...
export const createTheme = (themeInput?: DeepPartial<ITheme>): ITheme => {
  const rootFontSize =
  ...
  const baseTheme: ITheme = {
    palette: {
      ...
      getColor(codeOrColor: string) {
        const keyIndex = Object.keys(output.palette).indexOf(codeOrColor);
    ...
    typography: {
      ...
      h3: {
        color:
          cascade(themeInput && themeInput.typography, ['hero', 'h1', 'h2'], type => type.color) ||
          textColor,
        fontFamily:
          cascade(
  ...
  const output = deepmerge(baseTheme, themeInput || {}) as ITheme;
